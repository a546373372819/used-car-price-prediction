...
# Project-specific cleaning was moved here from make_subset.py.
...
# Relevant attributes for the project. The regression script keeps these columns
# when they exist in the input CSV, then performs the actual preprocessing.
PROJECT_COLUMNS = [
    "car_name",
    "yr_mfr",
    "fuel_type",
...
def extract_first_number(value):
    """Extracts the first numeric value from strings like '1,498 CC', '21.5 kmpl', '₹ 5.2 Lakh'."""
    if pd.isna(value):
        return np.nan
    ...
    # Replace commas inside numbers, but keep decimal dots.
    text = text.replace(",", "")
...
def preprocess_raw_dataframe(raw_df: pd.DataFrame, requested_target: str | None, sample_size: int | None):
    """
    Project-level data cleaning and preparation.

    The steps that used to be in make_subset.py are intentionally handled here:
    - clean column names,
    - keep the relevant project columns,
    - remove price-leakage columns,
    - remove rows without a valid sale_price,
    - remove exact duplicate rows.

    The remaining preprocessing that must be learned only from the training split
    is still performed later inside sklearn pipelines.
    """
    df = raw_df.copy()
    ...
    # 2) Keep only the project-relevant columns when this dataset contains them.
    #    The fallback keeps all columns, so the script still works if it is used
    #    on an already-prepared file with a slightly different column set.
    ...
    # 3) Remove columns that directly or almost directly reveal the target price.
    leakage_cols = [c for c in PRICE_LEAKAGE_COLS if c in df.columns and c != target_col]
    df = df.drop(columns=leakage_cols, errors="ignore")

    # 4) Remove invalid target values.
    #    sale_price must be numeric and positive for supervised regression.
    target_col = select_target_column(df.columns, requested_target)
    df[target_col] = df[target_col].map(extract_first_number)
    df = df.dropna(subset=[target_col])
    df = df[df[target_col] > 0]

    # 5) Remove exact duplicate rows after column selection and target cleaning.
    df = df.drop_duplicates().reset_index(drop=True)

    # Optional sampling is kept here only for quick local experiments.
    # The real project subset should normally be created with make_subset.py.
    ...
    # Drop columns that are usually identifiers or raw media/links/comments.
    drop_cols = []
    for col in df.columns:
        ...
        if any(hint in col for hint in DROP_NAME_HINTS):
            ...
            # Almost unique columns are usually IDs and can hurt generalization.
            drop_cols.append(col)
    df = df.drop(columns=sorted(set(drop_cols)), errors="ignore")

    # Create brand from a full car name when useful.
    # Create brand from a full car name when useful.
    ...
    # These features help the model capture interactions between important
    # categorical attributes. For example, the same body type does not have
    # the same price meaning for every manufacturer.
    ...
    if "make" in df.columns and "model" in df.columns:
        df["make_model"] = (
            clean_category_values(df["make"])
            + "_"
            + clean_category_values(df["model"])
        )
    ...
    if "make" in df.columns and "body_type" in df.columns:
        df["make_body_type"] = (
            clean_category_values(df["make"])
            + "_"
            + clean_category_values(df["body_type"])
        )

    if "fuel_type" in df.columns and "transmission" in df.columns:
        df["fuel_transmission"] = (
            clean_category_values(df["fuel_type"])
            + "_"
            + clean_category_values(df["transmission"])
        )

    # Convert bool-like categorical columns to 0/1.
    ...
    # Convert object columns with numeric/unit semantics.
    for col in list(df.columns):
        if col == target_col:
            continue
    ...
    # Feature engineering from common used-car columns.
    ...
    # In this dataset, the manufacturing year column is named "yr_mfr".
    # We explicitly create car_age from it because car age is one of the
    # strongest predictors of used car price.
    ...
    # Generic fallback for datasets that use other year column names.
    year_cols = [
    ...
    # Mileage per year: useful because the same mileage is not equally important
    # for a 2-year-old and a 10-year-old car.
    ...
    # Simple flags that can help tree-based models separate typical market segments.
    if "car_age" in df.columns:
        age = pd.to_numeric(df["car_age"], errors="coerce")
        df["car_age_squared"] = age ** 2
        df["is_newer_car"] = (pd.to_numeric(df["car_age"], errors="coerce") <= 3).astype(float)
        df["is_old_car"] = (pd.to_numeric(df["car_age"], errors="coerce") >= 10).astype(float)
